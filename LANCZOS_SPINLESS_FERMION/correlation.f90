...
program main
    ...
    integer startval, endval, channel
    ...
    double precision q, pi, Eg, win, arg, delta, w, JJ, s, fsgn !!
    ...
    call construct_hamiltonian(H0i,H0j,Hel0,H0len,maxH0len,basis0,hs_size0,ch)
    ...
    call construct_hamiltonian(H1i,H1j,Hel1,H1len,maxH1len,basis1,hs_size1,ch)
    ...
    Gs(:) = states(:,1)
    !  do i=1,hs_size0
    !     write(68,*) gs(i)  !ground state vector
    !  enddo
    ...
    startval = 4 !(N*rank)/msize+1;
    endval   = 4 !(N*(rank+1))/msize;
...
!! Evaluation of \sum_i e^{iq.R_i} S_i . S_J|g> and \sum_i e^{iq.R_i} S_i^z S_i . S_J|g>
    ...
    do ch =startval,endval
        ...
        nsite=7
        call remove_2particle(psi1,hs_size0,basis0,hs_size0,psi2,hs_size1,basis1,hs_size1,nsite,nsite+1,spin,N)
        psi0(:,ch) = psi2
    ...
    if(rank.eq.0) then
        ...
        deallocate(phi0)
        write(unit=filename,fmt="('2hole_specf_len',i2.2,'_partcle_',i1.2,'_U_',f4.2,'.dat')") N,nup, U
        ...
        do nw = 1,numw
            ...
            do i = 2,j
                ctmp = final(i)
                arg = w + Eg - EE(i)
                delta =  exp(-arg*arg/(2.0d0*sig*sig))/(sig*sqrt(2*pi)) 
               
                if(abs(Eg-EE(i)).gt.0.0001d0)then
                    AES(nw) = AES(nw) + delta*dreal(ctmp*conjg(ctmp))
                endif
            enddo
            write(60,*)w, AES(nw)
        enddo
